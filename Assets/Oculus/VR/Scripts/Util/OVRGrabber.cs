...
public class OVRGrabber : MonoBehaviour
{
    ...
    public bool isAddedOffset = false;
    ...
    protected virtual void GrabBegin()
    {
        ...
        if (closestGrabbable != null)
        {
            ...
            if (!isAddedOffset)
            {
                Debug.Log(m_grabbedObj.transform.name);

                if (m_grabbedObj.transform.name == "green")
                {
                    OffsetController.isAppliedOffest = true;
                    GameObject.Find("MainControl").GetComponent<OffsetController>().x = 0.175f;
                }
                else if (m_grabbedObj.transform.name == "blue")
                {
                    OffsetController.isAppliedOffest = true;
                    GameObject.Find("MainControl").GetComponent<OffsetController>().x = 0.125f;                    
                }
                else 
                {
                    OffsetController.isAppliedOffest = false;
                }
                
                OffsetController.startingPos = this.transform.position;
                OffsetController.isGrabbed = true;
                // this.transform.localPosition = new Vector3(0, hittingFaceControl.GetComponent<HittingFaceControl>().offset_0, 0);
                isAddedOffset = true;
            }
        }
    }
    ...
    protected void GrabEnd()
    {
        if (m_grabbedObj != null)
        {
            ...
            if (isAddedOffset)
            {
                this.transform.localPosition = new Vector3(0, 0.0f, 0);
                isAddedOffset = false;
                
                OffsetController.isGrabbed = false;
                OffsetController.endingLocalPos = mainControl.GetComponent<OffsetController>().rightController.transform.localPosition;
            }
        }
